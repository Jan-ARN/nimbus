...
- **Ensemble meteogram / bi-modality** (Long-range tab, 3rd toggle) — `src/components/EnsembleMeteogramChart.vue` + `bimodalSplit()` and `p25`/`p75`/`highs` on `EnsembleDay` in `series.ts`. Band | **Meteogram** | Scenarios: per-day box (p25–p75) + whisker (p10–p90) + median, with a **member dot-strip** (all 31 runs, deterministic jitter) — the part that actually reveals when the ensemble splits into two temperature camps (a smooth band hides it). A deliberately conservative detector (largest-gap split gated on separation ≥2, each camp ≥30%, gap ≥2.5°C) *only* drives a text callout + amber day-highlight — biased to false-negatives so a false "they split" never appears. Degrades gracefully: 0 bimodal days → honest "strong agreement" callout. Capped at the reliable window (~14 d). Detector unit-tested (`tsx`: fires on synthetic bimodal, rejects ramp/outlier/tiny-gap; 0/35 false positives on live heat-dome data); render branch verified in-browser via forced injection. *(§7)*
...
- Plain-language narration (§6) — rule-based paragraph over data already computed. Now especially cheap: it can fold in the trend, the anomaly (`climate.ts`), and the meteogram's bi-modality callout (`bimodalSplit` in `series.ts`) into one honest sentence.
